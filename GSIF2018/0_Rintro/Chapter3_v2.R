...
## R Script belong to Chapter 3 'Getting Spatial in R'
## B. Malone, B. Minasny, A.B. McBratney
## adapted by B. Kempen, ISRIC - World Soil Information
## version 2, 11 May 2017
##########################################################################################################################
...
## Raster plotting and point overlay
plot(r.DEM)
points(HV100, pch = 20)
...
### NOTE: THE CODE BELOW IS NOT INCLUDED IN THE BOOK CHAPTER. 
### It illustrates some raster processing functionality with the sp package
...
## convert to raster object with the 'gridded' function
gridded(HV_dem) <- ~X+Y
str(HV_dem)

## you see that the object is now a SpatialPixelsDataFrame class
## use the 'fullgrid'to convert it to a SpatialGridDataFrame class
fullgrid(HV_dem) <- TRUE

## check the projecion: you will that there is no projection associated to the object
proj4string(HV_dem)

## project the object
proj4string(HV_dem) <- CRS("+init=epsg:32756")
proj4string(HV_dem)

## write object to a GeoTiff file
writeGDAL(HV_dem["elevation"], fname = "elevation.tif", drivername = "GTiff", type = "Float32")

## read a GeoTiff file; the GeoTiff stores the projection and returns it automatically to the R object
hv_dem <- readGDAL(fname = "./elevation.tif")
proj4string(hv_dem) 

## alternatively, we can also project by using a character string that describes the reference system
data(HV_dem)
gridded(HV_dem) <- ~X+Y
# define object that stores the projection 
utm <- "+proj=utm +zone=56 +south +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0"
proj4string(HV_dem) <- CRS(utm)
proj4string(HV_dem)
